...
## 1. Versioning Format
- **Scheme**: Semantic Versioning (`MAJOR.MINOR.PATCH`) with a `v` suffix for git tags and release tracking (e.g., `2.6.0v`, `2.6.1v`).
- **Revision / Hotfix Suffixes (`a`, `b`, `c`)**: Attach lowercase letter suffixes to patch numbers for rapid iterative hotfixes or UI adjustments before incrementing the patch number (e.g., `2.6.1v` -> `2.6.1bv` -> `2.6.1cv`).

## 2. Commit & Git Tagging Conventions
- **Commit Messages**: All release/versioned commits must start with `<version>v - <Description>` (e.g., `2.6.1cv - Align landing page copy with actual system capabilities...`).
